          ...
          <div class="custom-table table3">
            ...
            <template #cell(imageQ)="data">
              <span v-if="data.item.imageQ != null">
                <b-avatar :src=" data.item.imageQ" size="4rem" rounded="lg" alt="" srcset="" />
              </span>
              <span v-else>
...
export default {
  metaInfo: { title: 'Question Requests' },
  data() {
    return {
      //table 3
      items2: [],
    ...
    Delete(id) {
      this.$swal({
        ...
        if (result.value) {
          ...
          this.$swal('Deleted!', 'The question request has been deleted.', 'success');
          this.$router.go();
        }
      });

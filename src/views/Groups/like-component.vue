<template>
  <span v-if="userprofile != null || userentreprise != null">
    <span v-if="liked == true">
      <svg @click="likedreply()" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
        <path
          fill="currentColor"
    ...
    <span v-else>
      <svg @click="deletedlike()" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
        <path
          fill="currentColor"
...
export default {
  ...
  computed: {
    ...mapGetters({
      Groupvotes: 'StateGroupvotes',
    }),
  },
  methods: {
    ...mapActions(['GetGroupvotes', 'CreateNotification', 'CreateGroupvote']),
    likedreply() {
      this.liked = false;

      axios.put('/postgroup/postgroup-update/' + this.postid + '/', { nblikes: this.likes - 1 });
      for (let v in this.Groupvotes) {
        if (this.Groupvotes[v].postVo === this.postid && (this.Groupvotes[v].userprofileVo === this.userprofile || this.Groupvotes[v].userentrepriseVo === this.userentreprise)) {
          axios.delete('/groupvote/groupvote-delete/' + this.Groupvotes[v].id + '/');
        }
      }
      this.likes -= 1;
    },
    deletedlike() {
      this.liked = true;
      if (this.userprofile != this.post.userprofilePost || this.userentreprise != this.post.userentreprisePost) {
      ...
      this.CreateGroupvote({ postVo: this.postid, userprofileVo: this.userprofile, userentrepriseVo: this.userentreprise });
      axios.put('/postgroup/postgroup-update/' + this.postid + '/', { nblikes: this.likes + 1 });
      this.likes += 1;
    },
  },
  ...
  created() {
    ...
    axios.get('/postgroup/postgroup-detail/' + this.postid + '/').then((response) => {
      this.post = response.data;
      this.likes = this.post.nblikes;
      for (let v in this.Groupvotes) {
        if (this.userprofile == this.Groupvotes[v].userprofileVo && this.Groupvotes[v].postVo == this.post.id) {
          this.liked = true;
        } else if (this.userentreprise == this.Groupvotes[v].userentrepriseVo && this.Groupvotes[v].postVo == this.post.id) {
          this.liked = true;
        }
      }
    });

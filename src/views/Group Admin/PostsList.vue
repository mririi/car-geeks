          ...
          <div class="page-header">
            <nav class="breadcrumb-one" aria-label="breadcrumb">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><span>Post List</span></li>
              </ol>
            </nav>
          </div>
        </li>
        ...
        <div class="panel p-0">
          <div class="custom-table table3">
            ...
            <template #cell(actions)="data">
              <span @click="DeletePost(data.item)">
                <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
            ...
            <template #cell(imagePost)="data">
              <span v-if="data.item.imagePost != null">
                <b-avatar :src=" data.item.imagePost" size="4rem" rounded="lg" alt="" srcset="" />
              </span>
              <span v-else>
  ...
  methods: {
    ...
    async DeletePost(post) {
       this.$swal({
        ...
        if (result.value) {
             axios.delete(`/postgroup/postgroup-delete/${post.id}/`);
             axios.put('/group/group-update/' + this.$route.params.id + '/', { nbposts: this.group.nbposts-1 });
            this.$router.go();
            this.$swal('Deleted!', 'Post deleted from your group.', 'success');

                ...
                <div class="panel br-6 p-0">
                    <div class="custom-table">
                        <div class="d-flex p-3 justify-content-between">
                            <div class="d-flex align-items-sm-center flex-column flex-sm-row mr-1">
                                <span class="text-nowrap mr-sm-2">Minimum age:</span>
                                <b-input v-model="min_age" size="sm" maxLength="3" placeholder="Minimum age..." @keyup.prevent="range_change()" />
                            </div>
                            <div class="d-flex align-items-sm-center flex-column flex-sm-row ml-1">
                                <span class="text-nowrap mr-sm-2">Maximum age:</span>
                                <b-input v-model="max_age" size="sm" maxLength="3" placeholder="Maximum age..." @keyup.prevent="range_change()" />
                            </div>
                        </div>
                        ...
                        <b-table ref="basic_table" responsive :items="items" :fields="columns" :per-page="table_option.page_size" :current-page="table_option.current_page" :filter="table_option.search_text"
...
<script>    
    export default {
        metaInfo: { title: 'Bootstrap Range Search Table' },
        data() {
            return {
                items_all: [], items: [], columns: [],
        ...
        methods: {
            ...
            range_change() {
                let dt = this.items_all;
                if (this.min_age != '' && this.min_age != null) {
                    dt = dt.filter(d => d.age >= Number(this.min_age));
                }
                if (this.max_age != '' && this.max_age != null) {
                    dt = dt.filter(d => d.age <= Number(this.max_age));
                }
                this.items = dt;
